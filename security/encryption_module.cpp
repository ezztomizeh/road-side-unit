...
std::string pad16(const std::string &data) {
    size_t pad = (16 - (data.size() % 16)) % 16;
    return data + std::string(pad, '\0');
}

std::string pad_pkcs7(const std::string &data) {
    size_t p = 16 - (data.size() % 16);
    return data + std::string(p, (char)p);
}

std::string unpad_pkcs7(const std::string &data) {
...
std::vector<block128> bytes_to_blocks(const std::string &data) {
    ...
    for (size_t i = 0; i < data.size(); i += 16)
        out.push_back({bytes_to_u64(data, i), bytes_to_u64(data, i+8)});
    return out;
}
...
public:
    CryptoSession(py::bytes key_bytes, py::bytes nonce_bytes) {
        ...
        if (k.size() != 16) throw std::runtime_error("Key must be 16 bytes");
    ...
    py::tuple encrypt(py::bytes plaintext, py::bytes aad_bytes) {
        std::string input = plaintext;
        std::string aad = aad_bytes;

        auto pt_blocks = bytes_to_blocks(pad_pkcs7(input));
        std::vector<block128> ct(pt_blocks.size());

        for (size_t i = 0; i < pt_blocks.size(); i++) {
        ...
        std::vector<block128> ghash_input;

        // AAD
        auto aad_blocks = bytes_to_blocks(pad16(aad));
        ghash_input.insert(ghash_input.end(), aad_blocks.begin(), aad_blocks.end());

        // Ciphertext
        ghash_input.insert(ghash_input.end(), ct.begin(), ct.end());

        // Length block
        block128 len_block = {
            (uint64_t)aad.size() * 8,
            (uint64_t)(ct.size() * 16) * 8
        };
        ghash_input.push_back(len_block);

        block128 tag = {0,0};
        for (auto &b : ghash_input)
            tag = gf_mul(tag ^ b, H);
    ...
    py::bytes decrypt(py::bytes ciphertext, py::bytes tag_bytes, py::bytes aad_bytes) {
        std::string ct_raw = ciphertext;
        std::string tag_in = tag_bytes;
        std::string aad = aad_bytes;

        auto ct = bytes_to_blocks(ct_raw);
        ...
        std::string expected = u64_to_bytes(tag.hi) + u64_to_bytes(tag.lo);

        if (expected != tag_in)
            throw std::runtime_error("Authentication failed");

        // ---- decrypt ----
        std::vector<block128> pt(ct.size());
        ...
        return py::bytes(unpad_pkcs7(blocks_to_bytes(pt)));
    }
};
